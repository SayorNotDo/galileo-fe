          ...
          <a-space v-else direction="vertical">
            ...
            <a-space>
              <a-avatar-group :size="24">
                {{ project.description }}
                <a-avatar
                  v-for="(contributor, idx) in project.involveMembers"
                  :key="idx"
                  :size="32"
                >
...
<script lang="ts" setup>
  import { queryUserProjectList } from '@/api/user';
  import { ProjectRecord } from '@/types/user';
  import useRequest from '@/hooks/request';

  const defaultValue = Array(6).fill({} as ProjectRecord);
  const { loading, response: projectList } = useRequest<ProjectRecord[]>(
